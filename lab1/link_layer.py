...
def const_transmission_error_probability_test(message_to_send: str, transmission_error_probability: int = 0.3,
    ...
    print(df_ws.to_latex(columns=['protocol', 'window_size', 'elapsed_time', 'k'], index=False))
...
def const_window_size_test(message_to_send: str, window_size: int = 5, tests_count: int = 4,
                           protocols: List[ProtocolT] = ['go_back_n', 'selective_repeat']):
    ...
    print(df_tep.to_latex(columns=['protocol', 'transmission_error_probability', 'elapsed_time', 'k'], index=False))
